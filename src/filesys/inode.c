  ...
  if (disk_inode != NULL)
    {
	  ...
	  //          block_write (fs_device, sector, disk_inode);
	      ...
	      //              static char zeros[BLOCK_SECTOR_SIZE];
              ...
              for (i = 0; i < sectors; i++) 
		{
		  block = cache_get_block(disk_inode->start + i, true);
		  data = cache_zero_block(block);
...
inode_open (block_sector_t sector)
{
  ...
  memcpy(&inode->data, data, BLOCK_SECTOR_SIZE);
  cache_put_block(block);
  ...
  while (size > 0) 
    {
	  ...
	  struct cache_block *block = cache_get_block(sector_idx, false);
	  void * data = cache_read_block (block);
	  memcpy (bounce, data, BLOCK_SECTOR_SIZE);
	  cache_put_block(block);
  ...
  while (size > 0) 
    {
	  ...
	  struct cache_block *block = cache_get_block (sector_idx, true);
	  void *data = cache_read_block (block);
	  memcpy (data, bounce, BLOCK_SECTOR_SIZE);
	  cache_mark_block_dirty (block);
	  cache_put_block (block);
